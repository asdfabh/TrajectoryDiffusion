...
def get_args_parser():
    ...
    parser.add_argument('--loss_time_power_fut', type=float, default=2.0, help="未来时序加权损失的时间幂次，>0 表示后段权重更高")
    parser.add_argument('--loss_end_weight_fut', type=float, default=2.0, help="终点误差项权重")
    parser.add_argument('--loss_vel_weight_fut', type=float, default=0.2, help="速度一致性误差项权重")
    parser.add_argument('--save_epoch_vis', action=argparse.BooleanOptionalAction, default=False, help="训练过程中每个 epoch 保存 train/val 可视化样本")
    parser.add_argument('--epoch_vis_sample_idx', type=int, default=0, help="每个 epoch 可视化的样本索引")
