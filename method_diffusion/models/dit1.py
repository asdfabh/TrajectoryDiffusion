...
class DiTBlock(nn.Module):
    ...
    def __init__(self, dim=128, heads=6, dropout=0.1, mlp_ratio=4.0, N=40, T=16):
        ...
        # AdaLN 调制参数 (时间 + MLP1 + 空间 + MLP2 = 4 组)
        self.adaLN_modulation = nn.Sequential(
            nn.SiLU(),
            nn.Linear(dim, 6 * dim, bias=True)  # 12 = 4 组 × 3 参数
        )
...
class FinalLayer(nn.Module):
    """
    预测所有车辆的未来轨迹 [B, N, pred_len, 2]
    """

    def __init__(self, hidden_size, N=40, T=16, output_dim=2):
        super().__init__()
        self.N = N
        self.T = T
        ...
        self.proj = nn.Sequential(
            ...
            nn.GELU(approximate="tanh"),
            nn.LayerNorm(hidden_size * 4),
            nn.Linear(hidden_size * 4, T * output_dim, bias=True)
        )
    ...
    def forward(self, x, y):
        ...
        # 提取每辆车的最后时间步特征
        x = x.view(B, self.N, T, D)[:, :, -1, :]  # [B, N, D]

        shift, scale = self.adaLN_modulation(y).chunk(2, dim=1)
        x = modulate(self.norm_final(x), shift, scale)

        x = self.proj(x)  # [B, N, T*2]
        # 修正:先reshape为 [B, N, T, output_dim],再调整维度顺序
        x = x.view(B, self.N, self.T, self.output_dim)  # [B, N, T, 2]
        x = x.permute(0, 2, 1, 3)  # [B, T, N, 2]
        # print(f"shape of x in final layer: {x.shape}")
