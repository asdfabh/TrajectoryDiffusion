...
class DiffusionFut(nn.Module):
    ...
    def forwardEvalMulti(self, hist, hist_nbrs, mask, temporal_mask, future, device, K=None):
        ...
        infer_timesteps = (np.arange(0, self.num_inference_steps) * step_ratio).round()[::-1].copy().astype(np.int64)
        infer_timesteps = torch.from_numpy(infer_timesteps).to(device)
        ...
        x_t = self.diffusion_scheduler.add_noise(anchor_x0, noise, trunc_timesteps).float()
        x_t = x_t.reshape(bsz * k, t_len, self.output_dim)

        pred_x0 = None
        for t in infer_timesteps:
