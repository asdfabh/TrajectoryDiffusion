...
class JointFutRefiner(nn.Module):
    def __init__(self, fut_model, refiner, args):
        super().__init__()
        self.fut_model = fut_model
        self.refiner = refiner
        self.train_fut = int(args.joint_train_fut) > 0
        self.fde_weight = float(args.fut_refiner_fde_weight)
        self.delta_weight = float(args.fut_refiner_delta_weight)
        self.gate_weight = float(args.fut_refiner_gate_weight)

    def forward(self, hist, hist_nbrs, mask, temporal_mask, fut, op_mask, device):
        if self.train_fut:
            all_preds = self.fut_model.forward_eval_multi(
                hist,
                hist_nbrs,
                mask,
                temporal_mask,
                device,
                K=self.fut_model.fut_k,
                stage="joint_train",
            )
        else:
            with torch.no_grad():
                all_preds = self.fut_model.forwardEvalMulti(
                    hist,
                    hist_nbrs,
                    mask,
                    temporal_mask,
        ...
        refined_all, aux = self.refiner(hist, all_preds, self.fut_model.fut_dt)
        return compute_refiner_loss(
            refined_all,
            fut,
            op_mask,
            aux,
...
def load_trainable_fut_model_for_rank(args, device, rank):
    if is_main_process(rank):
        return load_trainable_fut_model(args, device)
...
def train_epoch(args, joint_model, dataloader, optimizer, device, epoch, rank):
    joint_model.train()
    joint = unwrap_model(joint_model)
    joint.fut_model.eval()
    totals = {key: 0.0 for key in REFINER_STAT_KEYS}
    num_batches = 0
    pbar = tqdm(
        dataloader,
        total=len(dataloader),
        desc=f"Joint DDP Ep{epoch} Train",
        dynamic_ncols=True,
    ...
    for batch in pbar:
        ...
        loss, logs = joint_model(hist, hist_nbrs, mask, temporal_mask, fut, op_mask, device)
...
@torch.no_grad()
def evaluate(args, joint_model, dataloader, device, epoch, rank):
    joint_model.eval()
    joint = unwrap_model(joint_model)
    fut_model = joint.fut_model
    refiner = joint.refiner
    baseline_metrics = TrajectoryMetrics(fut_model.T)
    refined_metrics = TrajectoryMetrics(fut_model.T)
    pbar = tqdm(
        dataloader,
        total=len(dataloader),
        desc=f"Joint DDP Ep{epoch} Val",
        dynamic_ncols=True,
        disable=not is_main_process(rank),
    )
...
def main():
    ...
    if is_main_process(rank):
        print(f"[DDP JointRefinerTrain] Train path: {train_path}")
        print(f"[DDP JointRefinerTrain] Val path: {val_path}")
    ...
    fut_model = load_trainable_fut_model_for_rank(args, device, rank)
    fut_model.eval()
    ...
    joint_model = JointFutRefiner(fut_model, refiner, args).to(device)
    optimizer_groups = [
        {
            "params": joint_model.refiner.parameters(),
    ...
    if train_fut:
        optimizer_groups.append(
            {
                "params": joint_model.fut_model.parameters(),
    ...
    if is_main_process(rank):
        print(
            f"[DDP JointRefinerTrain] Training: fut_eval + refiner | "
    ...
    if dist.is_initialized():
        if device.type == "cuda":
            joint_model = DDP(joint_model, device_ids=[local_rank], output_device=local_rank, find_unused_parameters=False)
        else:
            joint_model = DDP(joint_model, find_unused_parameters=False)

    best_rmse = float("inf")
    for epoch in range(1, int(args.num_epochs) + 1):
        train_sampler.set_epoch(epoch)
        train_stats = train_epoch(args, joint_model, train_loader, optimizer, device, epoch, rank)
        baseline_summary, refined_summary = evaluate(args, joint_model, val_loader, device, epoch, rank)
